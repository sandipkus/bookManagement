  ...
  try {
    ...
    if (validator.isBodyExist(bookData)) {
      return res.status(400).send({
        status: false,
        message: "Data is required, Please provide book details.",
      });
    }
...
const getBook = async function (req, res) {
  try {
    ...
    if (!filters) {
      let books = await bookModel.find({ isDeleted: false }).select({
        _id: 1,
        title: 1,
        excerpt: 1,
        userId: 1,
      ...
      let books = await bookModel.find(filters).select({
        _id: 1,
        title: 1,
        excerpt: 1,
        userId: 1,
